...
> **SUPERSEDED (2026-07-10):** the design was amended by the user before execution and
> the implementation was completed against the revised spec
> (`docs/superpowers/specs/2026-07-10-railway-demo-design.md`). Key deltas vs. this
> plan: Perplexity research stays **live** (no `demo_research/` bake, no
> `scripts/bake_research.py`, zero `agents/` edits); the dataset is **32 real shipping
> lines** in a committed `demo_data/sales_actuals.csv` generated by
> `generate_fake_data.py` from the user's TM1 workbook (not `demo_data.py` at
> startup); strike-2 lockout disables the **entire session**; a single
> `requirements.txt` is kept (no `requirements-full.txt`). This file is retained for
> history only — do not execute it.
